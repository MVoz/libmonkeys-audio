...
int CAPECompress::Start(const wchar_t * pOutputFilename, const WAVEFORMATEX * pwfeInput, int64 nMaxAudioBytes, intn nCompressionLevel, const void * pHeaderData, int64 nHeaderBytes)
{
    if (m_pioOutput)
    {
        SAFE_DELETE(m_pioOutput)
    ...
    m_pBuffer = new unsigned char [uint32(m_nBufferSize)];
...
int64 CAPECompress::AddData(unsigned char * pData, int64 nBytes)
{
    ...
    while (nBytesDone < nBytes)
    {
        ...
        memcpy(pBuffer, &pData[nBytesDone], intn(nBytesToProcess));
...
int CAPECompress::ProcessBuffer(bool bFinalize)
{
    ...
    try
    {
        ...
        while ((m_nBufferTail - m_nBufferHead) >= nThreshold)
        {
            ...
            int nResult = m_spAPECompressCreate->EncodeFrame(&m_pBuffer[m_nBufferHead], intn(nFrameBytes));
            if (nResult != 0) { return nResult; }
        ...
        if (m_nBufferHead != 0)
        {
            ...
            if (nBytesLeft != 0)
                memmove(m_pBuffer, &m_pBuffer[m_nBufferHead], (size_t) nBytesLeft);
