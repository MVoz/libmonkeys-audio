...
uint32 CCircleBuffer::UpdateCRC(uint32 nCRC, intn nBytes)
{
	intn nFrontBytes = ape_min(m_nTail, nBytes);
	intn nHeadBytes = nBytes - nFrontBytes;

	if (nHeadBytes > 0)
		nCRC = CRC_update(nCRC, &m_pBuffer[m_nEndCap - nHeadBytes], int(nHeadBytes));

	nCRC = CRC_update(nCRC, &m_pBuffer[m_nTail - nFrontBytes], int(nFrontBytes));
